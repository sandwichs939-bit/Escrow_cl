...
def get_html_layout(deal):
    ...
    <body>
        <div class="card">
            ...
            <div class="amount-value">{deal['amount']} USDT</div>
            <div class="status-badge" id="statusText">{deal['status']}</div>
        </div>
        <div class="card">
            <div class="info-row"><span>Тип сделки:</span><span>Общие</span></div>
            <div class="info-row"><span>Комиссия:</span><span>5 USDT</span></div>
            <div class="info-row"><span style="color:#fec309; font-weight:bold;">Итого к оплате:</span><span style="color:#fec309; font-weight:bold;">{total} USDT</span></div>
        </div>
        <div class="card">
            <div style="margin-bottom:8px;"><b>Продавец:</b> {deal['seller']}</div>
            <div><b>Покупатель:</b> {deal['buyer']}</div>
        </div>
        ...
        <div class="wallet-box" id="walletBox">
            ...
            <div class="wallet-text" id="walletAddr">{WALLET_BEP20}</div>
        </div>
        <script>
